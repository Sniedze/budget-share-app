...
**Done (optional):** shared `buildBulkInsertPlaceholders`, `assertActiveGroupMembership` for settlement/update flows, `loadViewerGroupMemberName`, import UI types in `features/import/models.ts`, client `expenseAnalytics` unit tests.
...
**Stretch (done):** `server/src/db/sqlColumns.ts` for shared SELECT lists; `client/src/lib/localStorageJson.ts` + `workspaceStorageKeys.ts` for import/budget/auth storage; **pino** replaces hand-rolled JSON logger (`LOG_LEVEL` unchanged); membership/SQL placeholder DRY; import `models.ts`; dashboard analytics tests in CI.
